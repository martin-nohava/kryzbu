...
class Hmac_index:
    ...
    def add(file_name: str, hmac: str):
        ...
        if not Hmac_index.file_exists(file_name):
            con = sqlite3.connect(Hmac_index.FOLDER / Hmac_index.NAME)
            cur = con.cursor()
            cur.execute(f"INSERT INTO {Hmac_index.TABLE_NAME} VALUES (?,?)", (file_name, hmac))
            con.commit()
            con.close()
        else:
            # File with same name already exists
    ...
    def update(file_name: str, hmac: str):
        ...
        con = sqlite3.connect(Hmac_index.FOLDER / Hmac_index.NAME)
        cur = con.cursor()
        cur.execute(f"SELECT hmac FROM {Hmac_index.TABLE_NAME} WHERE name=:name", {"name": file_name})
